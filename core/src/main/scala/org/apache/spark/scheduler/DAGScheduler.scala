...
class DAGScheduler(
  ...
  var mapJobWaiter = scala.collection.mutable.Map[Int, JobWaiter[Any]]()

  def Stop(value: Int): Int = value
  ...
  def submitJob[T, U](
    ...
    if (partitions.size == 0) {
      ...
      val waiter = new JobWaiter[U](this, jobId, 0, resultHandler)
      mapJobWaiter(jobId) = waiter
      return waiter
    }
    ...
    if (jobId.isDefined) {
      ...
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        if (missing.isEmpty) {
          ...
          val waiter = mapJobWaiter(jobId.get)
          if (waiter.jobFinished == false) {
            var active = jobIdToActiveJob(jobId.get)
            var i = 0
